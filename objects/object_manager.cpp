...
bool object_manager::loadmesh( _mesh * mesh, int id,bool back){
...
bool object_manager::init(){
	if(!loadmesh( &m_cube_mesh   , IDR_CUBE   )){ return false; }
	if(!loadmesh( &m_sphere_mesh , IDR_SPHERE )){ return false; }
	if(!loadmesh( &m_mirror_cube_mesh   , IDR_CUBE   ,true)){ return false; }
	if(!loadmesh( &m_mirror_sphere_mesh , IDR_SPHERE ,true)){ return false; }
	return true;
}
